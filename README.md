...
You can run both consecutively and it will train both the first and second stages. The model will be saved in the format "epoch*1st*%05d.pth" and "epoch*2nd*%05d.pth". Checkpoints and Tensorboard logs will be saved at `log_dir`.
...
**_Before using these pre-trained models, you agree to inform the listeners that the speech samples are synthesized by the pre-trained models, unless you have the permission to use the voice you synthesize. That is, you agree to only use voices whose speakers grant the permission to have their voice cloned, either directly or by license before making synthesized voices public, or you have to publicly announce that these voices are synthesized if you do not have the permission to use these voices._**
...
This repo uses [PyTest](https://docs.pytest.org/en/7.4.x/) to run unit and integration tests.

#### About the Test Structure

The folder structure for this repo should follow the following recommended format:
...
In summary, this folder strucutre which enables the test files to be directly alongside the modules they are testing. This approach avoides the mega large root test folder for cleanliness and allows the tests to be closely associated with their corresponding code.

#### How to run tests

- `pytest`
  - Simply runs all tests across the project with any file naming starting with the convention test\*.py
- `PYTHONDONTWRITEBYTECODE=1 pytest`
  -adding PYTHONDONTWRITEBYTECODE=1 skips pycache generated
- `pytest --verbose`
  - Increases the verbosity of the output, which can be useful for getting more detailed information about the tests being run
- `PYTHONDONTWRITEBYTECODE=1 pytest --verbose`
  - Recommended.

Please see the Pytest documentation for the rest of the test flags but these are the recommended ones to use.
